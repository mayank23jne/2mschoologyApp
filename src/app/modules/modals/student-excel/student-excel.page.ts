...
@Component({
  selector: 'app-student-excel',
  templateUrl: './student-excel.page.html',
  styleUrls: ['./student-excel.page.scss'],
})
export class StudentExcelPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.fetch.getStudentExcel().subscribe({
      next: (res: any) => {
        if (res.code == 200) {
  ...
  submit(){
    ...
    this.fetch.studentExcel(this.formData).subscribe({
          next: (res: any) => {
            if (res.code == 200) {
